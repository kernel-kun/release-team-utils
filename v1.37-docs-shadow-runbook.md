...
A Docs Shadow helps the Docs Lead get the release-bound documentation and release notes ready for ship day. The work is light at the start of the cycle and intense in the final weeks; the running joke is "hurry up and wait."

### Recurring responsibilities

Our responsibilities span several recurring activities throughout the release:

**Weekly branch sync PR**

- _What:_ Each week the Release Docs Team opens a branch sync PR to merge the website's `main` branch into the upcoming release branch (`dev-1.37`).
- _Why:_ Syncing regularly keeps large merge conflicts from accumulating and makes the final release process much smoother.
- _How:_ See [§7 Weekly sync PR](#7-weekly-sync-pr-branch-sync).

**Release note PRs — creating and reviewing release notes**

- _What:_ Contributors add `release-note` blocks to PRs that introduce user-facing changes; these are collected and published as part of the official release notes.
- _Why:_ Any merged Kubernetes PR may carry a release note describing new features, bug fixes, removals, and so on. Thousands of PRs merge during a cycle and nobody can read them all, so SIG Release generates consolidated release notes.
- _How:_ We use `krel`, the SIG Release tooling. It queries GitHub for the KEPs' merged PRs, extracts the `release-note` blocks, and groups them. Step-by-step guidance is in [§8 krel core](#8-krel-core-release-notes).

**KEP follow-up** — reaching out to KEP owners to raise placeholder docs PRs before the docs deadlines.

**SIG Release and SIG Docs meetings** — giving status updates in the weekly Release Team meeting and the bi-weekly SIG Docs meeting.
...
| Docs Responsibility Sign-up Sheet (Excel)       | https://docs.google.com/spreadsheets/d/1B_kZtQMqjt4oLGAcVi180TEyr_ZNe-8wz9He7c67W9w/edit?usp=sharing       |
...
| v1.37 Release Meeting Agenda                    | https://docs.google.com/document/d/19xTpCJ_dSKDNI0IyOjyDqFnkmVGEP4rUwqKRgRtE7Ac/                           |
| SIG Docs Meeting Agenda                         | https://docs.google.com/document/d/1emuO4nmaQq3K8JZ9-MQeIygtrCPO9kWv7U7RzTaW4F8/                           |
...
gantt
   ...
   Periodic krel runs plus --fix :active, r1, 2026-06-10, 2026-08-05
...
1. [Manual steps](https://github.com/kubernetes/sig-release/blob/master/release-team/role-handbooks/docs/Release-Timeline.md#weekly-branch-sync-prs)
2. Using the helper script: [`branch-sync.sh`](https://github.com/kubernetes/sig-release/tree/master/release-team/role-handbooks/docs/branch-sync-script).
...
- v1.36: [website#55104 — Merge main into dev-1.36 to keep in sync](https://github.com/kubernetes/website/pull/55104)
